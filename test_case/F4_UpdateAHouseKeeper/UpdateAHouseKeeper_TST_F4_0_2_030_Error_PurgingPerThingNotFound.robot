...
*** Test Cases ***
################### Post ###################
UpdateAHouseKeeper_TST_F4_0_2_030_Error_PurgingPerThingNotFound
	...
	${valueBindingType}=    Set Variable    ${VALUE_BINDINGTYPE_PREACCOUNT}
	...
	${valueBindingTypeUpdate}=    Set Variable    ${VALUE_BINDINGTYPE_PRETHING}
	...
	#Body
    ${body}=    Evaluate    {"Type":"${valueType}","ConditionProperties":[{"Type":"${typeConditionProperties}","Value":${valueConditionPropertiesUpdate}}],"BindingType":"${valueBindingTypeUpdate}","BindingKey":"${valueBindingKey}","Name":"${HouseKeeperName}"}
    ...
    ${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have Error    ${res}    ${UPDATEAHOUSEKEEPER}   ${FIELD_OPERATIONSTATUS}    ${FIELD_CODE}    ${FIELD_DEVELOPERMESSAGE}    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}
	...
	${bodyLog}=    Set Variable    {"Type":"${valueType}","ConditionProperties":[{"Type":"${typeConditionProperties}","Value":${valueConditionPropertiesUpdate}}],"BindingType":"${valueBindingTypeUpdate}","BindingKey":"${valueBindingKey}","Name":"${HouseKeeperName}"}

	#====== Check log and Verify DB ==========

	#Check log detail and summary
    Log UpdateAHouseKeeper Error    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${res}    ${pathUrl}    ${dataSearch}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}    ${accessToken}    ${AccountId}    ${bodyLog}

    [Teardown]    Generic Test Case Teardown    ${UPDATEAHOUSEKEEPER_PARTNER}    ${createResponse}    ${HouseKeeperId}
