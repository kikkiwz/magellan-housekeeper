...
*** Test Cases ***
################### Post ###################
CreateAHouseKeeper_TST_F2_0_2_050_Error_KeepingTypeDuplicated
    ...
    ${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have Error    ${res}    ${CREATEAHOUSEKEEPER}   ${FIELD_OPERATIONSTATUS}    ${FIELD_CODE}    ${FIELD_DEVELOPERMESSAGE}    ${VALUE_LOG_CODE_40301}    ${VALUE_DESCRIPTION_THETYPECOULDNOTBEDUPLICATED_ERROR}
    ...
    Log CreateAHouseKeeper Error    ${VALUE_LOG_CODE_40301}    ${VALUE_DESCRIPTION_THETYPECOULDNOTBEDUPLICATED_ERROR}    ${VALUE_LOG_CODE_40301}    ${VALUE_DESCRIPTION_THETYPECOULDNOTBEDUPLICATED_ERROR}    ${res}    ${pathUrl}    ${dataSearch}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}    ${accessToken}    ${AccountId}    ${bodyLog}
