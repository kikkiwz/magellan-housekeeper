...
*** Test Cases ***
################### Post ###################
CreateAHouseKeeper_TST_F2_1_1_012_Success_KeepingPerThingDateTime
	...
	${typeConditionProperties}=    Set Variable    ${VALUE_CONDITIONPROPERTIES_TYPE_DATETIME}
	${startDate}=    Set Variable    ${VALUE_HOUSEKEEPER_DATETIME_STARTDATE}
	${endDate}=    Set Variable    ${VALUE_HOUSEKEEPER_DATETIME_ENDDATE}

	#Body
    ${body}=    Evaluate    {"Type":"${valueType}","ConditionProperties":[{"Type":"${typeConditionProperties}","StartDate":"${startDate}","EndDate":"${EndDate}"}],"BindingType":"${valueBindingType}","BindingKey":"${valueBindingKey}","Name":"${HouseKeeperName}"}
	...
	Run keyword And Continue On Failure    Should Be Equal As Strings    ${GetResponse_ConditionProperties}    [{'Type': '${typeConditionProperties}', 'StartDate': '${startDate}', 'EndDate': '${EndDate}'}]
	...
	${dataConditionPropertiesDB}=    Set Variable    [{'Type': '${typeConditionProperties}', 'StartDate': '${startDate}', 'EndDate': '${EndDate}'}]

    ${dataConditionPropertiesLog}=    Set Variable    [{"Type":"${typeConditionProperties}","StartDate":"${startDate}","EndDate":"${EndDate}"}]
    ...
    [Teardown]    Generic Test Case Teardown    ${PARTNERACCOUNTTHINGARRCORECREATEAHOUSEKEEPER}    ${createResponse}    ${GetResponse_HouseKeeperId}
